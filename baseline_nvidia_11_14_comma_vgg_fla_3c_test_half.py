...
log_dir="log/5_19comma_vgg_fla_pw_3c_kivinet_2pred_test_half_6th/"
...
pretrained_model = "log/5_19comma_vgg_fla_3c_kivinet_2pred_test_half_6th/KivlNet_part2_20.h5"
preWhiten = False
...
def extract_one_batch(list_train_index, train_dir, shape_img = [rows,cols,channels]):
    ...
    for index_file_name,file_name in enumerate(list_train_index):
        ...
        img_vgg = cv2.imread(vgg_cam_dir+"/"+str(file_name)+img_last_name)#[:,:,0:1]/255
        img_vgg = cv2.resize(img_vgg,(cols,rows))/255        
