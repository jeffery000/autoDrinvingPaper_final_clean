...
for epoch in range(epochs_initial+epochs_finetune):
    ...
    for batch_id in range(num_batch):
        current_batch = extract_one_batch(train_read_index[batch_id*batch_size:(batch_id+1)*batch_size],\
            train_dir)
        mbv2_self_driving.fit(current_batch, label_train[batch_id*batch_size:(batch_id+1)*batch_size],\
        ...
        if batch_id%10 == 0:
                for batch_test_id in range(num_batch):
                    current_batch = extract_one_batch(test_read_index[batch_test_id*batch_size:(batch_test_id+1)*batch_size],\
                        train_dir)
                    mbv2_self_driving.evaluate(current_batch, label_test[batch_test_id*batch_size:(batch_test_id+1)*batch_size],\
                                    batch_size=batch_size,\
                                    verbose=1)


#%%
mbv2_self_driving.fit(a,)
mbv2_self_driving.
keras.optimizers.Adam(learning_rate=d,)