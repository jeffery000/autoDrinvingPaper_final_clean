...
def visualize_feature_map(img_batch,fig_name):
    ...
    feature_map_sum_grey = 0-feature_map_sum
    cv2.imwrite(fig_name+"_sum"+".jpg", normlization(feature_map_sum_grey)*255)
    plt.imshow(feature_map_sum_grey,cmap="Greys")
    axis('off')
    plt.savefig(fig_name+"_sum"+".png")
 
model_name = "log/4_26_30ep_comma_3c_kivinet_2pred_test_half/KivlNet_part2_25.h5"
# model_name = "log/4_27udacity_cnn_vis/KivlNet_part2_15.h5"
output_layer_name = ["activation_1","activation_2","activation_3","activation_4"]#'activation_1'
img_name = r'E:\dateset\comma_c12_uisee_100_102_104_105_115_118_m\img\7494.jpg'
# img_name = r'E:\dateset\selfdriving-car-simulator_Uisee1_3\img\556.jpg'
...
conv_img2 = model2.predict(img_batch)   #1*row*col*32
conv_img3 = model3.predict(img_batch)   #1*row*col*32
conv_img4 = model4.predict(img_batch)   #1*row*col*32
...
visualize_feature_map(conv_img3,"conv3")
visualize_feature_map(conv_img4,"conv4")
# visualize_feature_map(conv_img2,"conv2")
plt.show()